...
class MyRobot(Robot):
    ...
    def look_at_face(self, display=False):
        img = self.camera_read_external()
        ...
        for face in faces:  # actually the robot just looks at the last detected face or skips the for-loop if faces is empty
            x,y,w,h = face
            fx, fy = x+int(w/2), y+int(h/2)
            face_center = self.compute_face_center(fx, fy, img)
            self.look_at(face_center, velocity=1.)
        if display:
    ...
    def run_image_reader(self, path):
    ...
    # Moves NAO arms following pre recorded motions. Moves head following person position from webcam
    def recorded_motion(self, L_timestamp, R_timestamp):
        
        cTime = time.time()
        fps = 1 / (cTime - self.pTime)
        self.pTime = cTime
        
        self.move_left_elbow(L_timestamp)
        self.move_right_elbow(R_timestamp)
            

    
    # Moves NAO arms and head following person position from webcam
    def pose_estimation(self):
        ...
        if lmList:            
            ...
            L_timestamp = self.unpack_arm_values(lmList.get(11), lmList.get(13), lmList.get(15))
            R_timestamp = self.unpack_arm_values(lmList.get(12), lmList.get(14), lmList.get(16))
    ...
    # In the order shoulder, elbow, wrist
    def unpack_arm_values(self, lm_shoulder, lm_elbow, lm_wrist): 
        x_sh = lm_shoulder.x
        y_sh = lm_shoulder.y
        z_sh = lm_shoulder.z
        ...
        timestamp = [0, x_sh, y_sh, z_sh, x_el, y_el, z_el, x_wr, y_wr, z_wr]
    ...
    def move_left_elbow(self,L_timestamp):
        pi = np.pi
    ...
    # Experimental method 
    def move_right_elbow(self, R_timestamp): 
        pi = np.pi
...
# robot = Nao(ext_camera_flag = True)
# robot.run_keyboard()
